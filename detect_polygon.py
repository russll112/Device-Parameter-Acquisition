...
from utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, apply_classifier, \
    scale_coords, xyxy2xywh, strip_optimizer, set_logging, increment_path, polygon_non_max_suppression, \
    polygon_scale_coords
...
def detection(image_path):
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='weights/best_284.pt', help='model.pt path(s)')
...
def detect(opt, save_img=False, view_img=True):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # Apply NMS
        pred = polygon_non_max_suppression(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes,
                ...
                for *xyxyxyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        polygon_plot_one_box(torch.tensor(xyxyxyxy).cpu().numpy(), im0, label=label,
                                             color=colors(c, True), line_thickness=opt.line_thickness)
